...
import { getGuardians, deleteGuardianById } from 'apiSdk/guardians';
import { GuardianInterface } from 'interfaces/guardian';

type ColumnType = ColumnDef<GuardianInterface, unknown>;

interface GuardianListPageProps {
...
export function GuardianListPage(props: GuardianListPageProps) {
  const {
    filters = {},
    titleProps = {},
  ...
  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<GuardianInterface>>(
    () => `/guardians?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: GuardianInterface) => {
    if (hasAccess('guardian', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/guardians/view/${row.id}`);
    }
  };
      ...
      ? {
          ...
          cell: ({ row: { original: record } }: any) => (
            <Flex justifyContent="flex-end">
              <NextLink href={`/guardians/view/${record.id}`} passHref legacyBehavior>
                <Button
              ...
              {hasAccess('guardian', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
                <NextLink href={`/guardians/edit/${record.id}`} passHref legacyBehavior>
                  <Button
                    onClick={(e) => e.stopPropagation()}
                    mr={2}
                    padding="0rem 0.5rem"
              ...
              {hasAccess('guardian', AccessOperationEnum.DELETE, AccessServiceEnum.PROJECT) && (
                <IconButton
                  onClick={(e) => {
                    e.stopPropagation();

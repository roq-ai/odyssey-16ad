...
import { getGuardians } from 'apiSdk/guardians';
...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['guardian-user', 'student-user'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<GuardianInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/guardians` : null),
    () => getGuardians({ tenant_id: session?.user?.tenantId }).then(({ data }) => data),
  );
  loading = isLoading;
  ...
  const MockedLinkItems: Array<NavItemPropsInterface> = [
    ...
    {
      name: 'Guardians',
      path: isTenantUser ? `/guardians/view/${data?.[0]?.id}` : '/guardians',
      entity: 'guardian',

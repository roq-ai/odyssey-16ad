...
const notificationMapping: Record<string, NotificationConfigInterface> = {
  'student.update': {
    roles: ['guardian-user'],
    key: 'student-progress-update',
    tenantPath: ['guardian', 'user', 'student'],
    userPath: [],
  },
  'lesson.create': {
    roles: ['student-user'],
    key: 'new-lesson-assigned',
    tenantPath: ['guardian', 'user', 'student', 'lesson'],
    userPath: [],
  },
  'feedback.create': {
    roles: ['student-user'],
    key: 'feedback-received',
    tenantPath: ['guardian', 'feedback'],
    userPath: [],
  },
};

const ownerRoles: string[] = ['guardian-user'];
const customerRoles: string[] = [];
const tenantRoles: string[] = ['student-user', 'guardian-user'];
...
export async function notificationHandlerMiddleware(req: NextApiRequest, entityId: string) {
  ...
  const tenant = await prisma.guardian.findFirst({
